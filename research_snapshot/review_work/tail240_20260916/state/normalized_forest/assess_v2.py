...
import run_f3_v2 as parallel

common, np, pd = subject.common, subject.np, subject.pd
OUT = common.external_path(subject.OUT.parent / 'assessment_v2')
FOLD_FOLDERS = {'F1':subject.OUT/'F1', 'F3':parallel.OUT/'F3'}
...
def declare():
    ...
    value = dict(source_sha256=common.sha256(__file__),
        unchanged_gate_source_sha256=common.sha256(Path(__file__).with_name('assess.py')),
        fold_folders={f:str(p) for f,p in FOLD_FOLDERS.items()},
        producer_protocol_sha256={f:common.sha256(p.parent/'protocol.json') for f,p in FOLD_FOLDERS.items()},
...
def assess():
    ...
    records = {}
    for fold in ['F1', 'F3']:
        folder = FOLD_FOLDERS[fold]
        manifest = common.read_json(folder / 'manifest.json')
        assert manifest['status'] == 'complete'
        assert manifest['protocol_sha256'] == declaration['producer_protocol_sha256'][fold]
