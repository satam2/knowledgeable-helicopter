...
OUT=common.external_path(ROOT/'private_runs/tail240_20260916/state/final_ple387/model_v2')
...
def declare():
    ...
    record=dict(previous_protocol_sha256=common.sha256(OUT.parent/'model_v1/protocol.json'),
        resource_only_change='Parentauthorized startup30GiB after two32GiB holds;24GiBprocesscap and8GiBdynamicreserve unchanged;allscientificparameters/19epochs/source/input contracts unchanged;nootherheavyfit.',
        ...
        resources='ExclusiveGPU,2CPU,<24GiBcurrent/historicalprocesspeak,start>=30GiBavailable,>=8GiBhostreserve;guardload/prepare and every200optimizersteps. No otherfit beforeparentallocation.',
...
def run():
    protocol=declare()
    assert psutil.virtual_memory().available>=30*1024**3
