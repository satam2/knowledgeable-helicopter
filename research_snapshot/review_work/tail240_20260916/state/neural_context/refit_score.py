...
OUT = common.external_path(ROOT / 'private_runs/tail240_20260916/state/neural_context/refit_score_v1')
...
def declare():
    ...
    payload = dict(created_from='ExposedJune/Octtune matchedfamilyinformationqualification',
        ...
        variants=dict(replacement='global9 + frozenPLEweight*(new387-old225) onordinaryonly',
            blend25='.75global9+.25new387 onordinaryonly'),
        protected='Allnonordinary rows includingmissingNM andfinitetail exactlyfrozenglobal9; allotherexpertweightsandpredictions unchanged. No scoreselectedweights/variants.',
...
def run(fold):
    ...
    assert split == original_marker['split']
    ...
    base_path = ENSEMBLE / f'{fold}_score/global9.parquet'
    assert common.sha256(base_path) == ensemble_marker['outputs']['global9.parquet']
