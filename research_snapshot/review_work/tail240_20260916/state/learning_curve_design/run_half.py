...
OUT=common.external_path(ROOT/'private_runs/tail240_20260916/state/learning_curve_design/v1')
...
def selection():
    ...
    if path.exists():
        ...
        assert {k:v for k,v in old.items() if k not in ignored}=={k:v for k,v in receipt.items() if k not in ignored}
...
def run():
    ...
    np.testing.assert_array_equal(stored[ID],parts['tune'][ID])
    ...
    labels=pd.read_parquet(META,columns=[ID,TARGET]).set_index(ID)[TARGET]
    ...
    np.testing.assert_array_equal(stored[TARGET],ytune)
