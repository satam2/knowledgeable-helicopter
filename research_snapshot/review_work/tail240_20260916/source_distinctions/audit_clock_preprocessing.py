"""Observed-only early-fit clock reconstruction and numeric semantic checks."""
import lightgbm
...
OUT = common.external_path(ROOT / 'private_runs/tail240_20260916/source_distinctions/clock_preprocessing_audit_v1')
...
def main():
    ...
    for path in paths:
        ...
        for name, end, start in pairs:
            ...
            expected = seconds.astype(np.float32).astype(float)
            ...
            comparisons[name] = dict(rows=len(seconds), finite=int(valid.sum()),
                mismatches=0, finite_to_nonfinite=int(np.sum(valid & ~np.isfinite(expected))),
                ...
                max_float32_rounding_sec=float(np.max(np.abs(expected[valid]-seconds[valid]))) if valid.any() else 0.)
