...
def main():
    ...
    parser.add_argument('--family',choices=['rotation','catboost'],required=True)
    ...
    for fold in ('F1','F3'):
        ...
        model_records={}
        for arm in arms:
            directory=root/fold/arm if family=='catboost' else root/fold
        ...
        if family=='catboost':
            ...
            np.testing.assert_array_equal(previous[ID],ids)
            controls={'unscaled':predictions['unscaled'],'scaled_lightgbm':previous.prediction_sec.to_numpy()}
            candidate=predictions['observed_schedule_scale']
    ...
    assert advance==summary['advance' if family=='catboost' else 'gate_passed']
