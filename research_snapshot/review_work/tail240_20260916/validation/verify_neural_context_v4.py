...
def bounded_load(ids,nfit,desired,folder):
    original_schema=pq.read_schema
    schemas={}
    def cached_schema(path,*args,**kwargs):
        key=str(path)
        if key not in schemas:schemas[key]=original_schema(path,*args,**kwargs)
        return schemas[key]
    try:
        pq.read_schema=cached_schema
        sources=producer.risk.feature_sources(desired)
    finally:
        pq.read_schema=original_schema
    schemas.clear();gc.collect()
...
def main(fold,version):
    out=ROOT/f'private_runs/tail240_20260916/validation/neural_context_{fold}_{version}_verifier_v4'
