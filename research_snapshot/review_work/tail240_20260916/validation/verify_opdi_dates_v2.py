...
OUT = ROOT/'private_runs/tail240_20260916/validation/opdi_dates_oracle_v2'
...
def main():
    ...
    for row in w.itertuples(index=False):
        ...
        assert pub.id.nunique()==1
        pub=pub.iloc[0]
        repairs.append({ID:entry[ID],'mode':row.mode,'shift':row.shift,'public_id_exact':str(int(pub.id)),
            'public_id_rounded':row.public_id,'raw_id_differs_from_rounded':int(pub.id)!=int(row.public_id)})
