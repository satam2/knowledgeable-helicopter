...
BASE = previous.BASE
OUT = v.ROOT / 'private_runs/tail240_20260916/validation/surface_takeoff_v1'
...
def main():
    ...
    folder = BASE / 'takeoff_join_v1'
    marker = v.read_json(folder / 'manifest.json')
    protocol = v.read_json(folder / 'protocol.json')
    assert marker['status'] == 'complete'
    assert marker['source_sha256'] == protocol['source_sha256'] == v.sha256(v.ROOT / 'review_work/tail240_20260916/forensics/surface_takeoff_join_v1.py')
    ...
    events = pd.read_parquet(BASE / 'inspection_v2/airport_events.parquet')
    ...
    anchor = anchor.loc[anchor.event_airport.eq(anchor.adep) & anchor.event_time.notna()]
    ...
    for row in query.itertuples(index=False):
        ...
        record = dict(candidate_count=len(matches), flight_id_exact='', anchor_event_id_exact='', anchor_event_time=pd.NaT, anchor_minus_takeoff_sec=np.nan, anchor_minus_firstseen_sec=np.nan, anchor_within_segment=False)
        if len(matches) == 1:
            hit = matches[0]
            record.update(flight_id_exact=str(hit.flight_id_exact), anchor_event_id_exact=str(hit.event_id_exact), anchor_event_time=hit.event_time, anchor_minus_takeoff_sec=(hit.event_time-now).total_seconds(), anchor_minus_firstseen_sec=(hit.event_time-hit.first_seen).total_seconds(), anchor_within_segment=hit.first_seen <= hit.event_time <= hit.last_seen)
        rows.append(record)
    ...
    expected['anchor_event_time'] = pd.to_datetime(expected.anchor_event_time, utc=True)
