"""Independent NM436 replay and current387 ensemble composition."""
import os
...
BASE=ROOT/'private_runs/tail240_20260916/models/nm_clock_peers_tune_v2'
pa.set_cpu_count(1)
...
def main(fold):
    ...
    folder=BASE/fold
    out=ROOT/'private_runs/tail240_20260916/validation'/('nm_clock_peers_'+fold)
    ...
    assert marker['source_sha256']==protocol['source_sha256']==v.sha256(ROOT/'review_work/tail240_20260916/models/nm_clock_peers_tune.py')
    ...
    assert columns==protocol['columns'] and len(columns)==436
    ...
    peerroot=ROOT/'private_runs/tail240_20260916/forensics/nm_clock_peers/cache_v2'
    assert v.sha256(peerroot/'protocol.json')==protocol['peer_protocol_sha256']
    peermarker=v.read_json(peerroot/'manifest.json')
    assert peermarker['status']=='complete'
    assert v.sha256(peerroot/'features.parquet')==peermarker['outputs']['features.parquet']
    peer=pd.read_parquet(peerroot/'features.parquet',filters=[(ID,'in',ids.tolist())]).set_index(ID).loc[ids]
    assert list(peer)==columns[-49:]
    expected=peer.to_numpy('float32');expected[~np.isfinite(expected)]=-999999.
    np.testing.assert_array_equal(matrix[:,-49:],expected)
    ...
    receipt=dict(status='passed',source_sha256=v.sha256(__file__),manifest_sha256=v.sha256(folder/'manifest.json'),protocol_sha256=v.sha256(BASE/'protocol.json'),cohorts=marker['ids'],all_native_predictions_exact=True,all_fit_vocabularies_exact=True,all_forty_nine_peer_features_exact=True,current387_baseline_composition_exact=True,control_matches_original_union_and_global9=True,matched=matched,ordinary_fixed25=comparison,peak_bytes=guard(),limitation='NMpeer source membership independently audited previously; this run checks all49 cache joins, fit vocabularies and native candidate predictions. Original control vector hash/parity checked; control model not rerun here.')
