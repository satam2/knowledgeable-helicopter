...
BASE=ROOT/'private_runs/tail240_20260916/forensics/alias_rotation/pilot_v2'
OUT=ROOT/'private_runs/tail240_20260916/validation/alias_rotation_v2'
...
def query_code(value,mode,mapping):
    ...
    match=re.fullmatch(r'([A-Z]+)([0-9]+)([A-Z]*)',clean)
    if match is None:
        return clean
...
def main():
    ...
    assert manifest['source_sha256']==sha(ROOT/'review_work/tail240_20260916/forensics/alias_rotation_v2.py')
    ...
    for mode in ('learned','lexical'):
        ...
        coverage[mode+'_total_matches']=int(features[f'alias_{mode}_match'].sum())
        coverage[mode+'_total_rotations']=int(features[f'alias_{mode}_previous_same_airport'].sum())
        coverage[mode+'_lost_raw_matches']=int((features[f'alias_{mode}_match'].eq(0)&features.alias_raw_match.eq(1)).sum())
        coverage[mode+'_lost_raw_rotations']=int((features[f'alias_{mode}_previous_same_airport'].eq(0)&features.alias_raw_previous_same_airport.eq(1)).sum())
