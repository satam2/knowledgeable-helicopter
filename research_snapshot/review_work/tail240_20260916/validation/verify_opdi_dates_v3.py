...
OUT = ROOT/'private_runs/tail240_20260916/validation/opdi_dates_oracle_v3'
...
def main():
    ...
    for source in acquisition['sources']:
        ...
        for batch in pq.ParquetFile(path).iter_batches(batch_size=65536,columns=fields,use_threads=False):
            ...
            frame['id_exact_str']=frame.id.map(lambda value:str(int(value)))
            frame['public_month']=source['month']
            if source['month'].startswith('2025'):
                witness_rows.append(frame.loc[frame.id.astype('float64').isin(wanted_ids)])
    ...
    witness_lookup={key:group for key,group in witnessed.groupby(witnessed.id.astype('float64'),sort=False)}
    repairs=[]
    ...
    for row in w.itertuples(index=False):
        ...
        pub=pub.loc[pub.adep.eq(str(query.ADEP_mvt).upper())&pub.ades.eq(str(query.ADES_mvt).upper())]
        pub=pub.loc[pub.flt_id.map(lambda value: ''.join(code_parts(value)) if code_parts(value) is not None else None).eq(row.code)]
        assert pub.id_exact_str.nunique()==1
        pub=pub.iloc[0]
        repairs.append({ID:entry[ID],'mode':row.mode,'shift':row.shift,'public_id_exact':pub.id_exact_str,
            'public_id_rounded':row.public_id,'raw_id_differs_from_rounded':int(pub.id_exact_str)!=int(row.public_id)})
    ...
    for movement_id,query in sample.iterrows():
        ...
        for mode in ('raw','learned','lexical'):
            ...
            for shift in (-1,0,1):
                ...
                hypotheses+=1
    repair_path=OUT/'recovered_exact_witness_ids.parquet'
    pd.DataFrame(repairs).to_parquet(repair_path,index=False)
    receipt={'status':'passed_feature_oracle_with_witness_id_rounding_defect','source_sha256':sha(__file__),'producer_manifest_sha256':sha(BASE/'manifest.json'),
        ...
        'rounded_public_id_witnesses':int(sum(row['raw_id_differs_from_rounded'] for row in repairs)),
        'recovered_exact_witness_ids_sha256':sha(repair_path),
        'limits':'Producer public_id is float64 and loses uint64 precision; exact raw witness IDs recovered uniquely via rounded ID plus route/code/exact timestamps. Original outputs unchanged. No taxi labels/private block clocks read. All witness arithmetic checked; candidate completeness and abstentions brute-forced for deterministic samples only. Mapping chronology source-reviewed; no independent prefix-support recount. Observedtrackstart is not offblock or proof of identity/date correction.'}
