...
OUT=ROOT/'private_runs/tail240_20260916/validation/opdi_known_sample_v2'
            ...
            for slice_name,query in [('all_finite',selected),('ordinary',selected.loc[selected.proxy_sec.between(0,7200)])]:
                ...
                for mode in ['raw','learned','lexical']:
                    ...
                    for group in ['resolved','unique']:
                        ...
                        np.testing.assert_array_equal((pd.to_datetime(q[v.common.MOVEMENT],utc=True).reset_index(drop=True)-pd.to_datetime(w.first_seen,utc=True).reset_index(drop=True)).dt.total_seconds().to_numpy(),offset)
