...
spec=importlib.util.spec_from_file_location('normalized_lexical_source',ROOT/'review_work/tail240_20260916/forensics/run_normalized_lexical_v2.py')
producer=importlib.util.module_from_spec(spec)
spec.loader.exec_module(producer)
BASE=producer.OUT
OUT=ROOT/'private_runs/tail240_20260916/validation/normalized_lexical_v2'
...
def main():
    ...
    assert sha(BASE/'lexical.parquet')==cache['feature_sha256']
    context=pd.read_parquet(BASE/'lexical.parquet').set_index(ID)
    ...
    assert list(context)==protocol['lexical_columns'] and len(x.columns)==76
    oldcolumns=read(ROOT/'private_runs/tail240_20260916/state/models_v2/control/F1/manifest.json')['feature_columns']
    assert list(x)==oldcolumns
    frozen_path=producer.LEXICAL/'all_missing_features.parquet'
    assert sha(frozen_path)==cache['frozen_lexical_source_cache_sha256']
    frozen=pd.read_parquet(frozen_path).set_index(ID)[protocol['lexical_columns']]
    pd.testing.assert_frame_equal(context,frozen,check_exact=True)
    fresh=producer.lexical_features(producer.decode_flight_tokens(x.flight))
    pd.testing.assert_frame_equal(context,fresh,check_exact=True)
    compared=len(context)*len(context.columns)
    ...
    for fold in ('F1','F3'):
        ...
        days=meta.iloc[rows['tune']][producer.base.TIME].dt.strftime('%Y-%m-%d').to_numpy()
        comparison=validation.paired(target,predictions['control76'],predictions['lexical91'],days)
        reported=read(BASE/fold/'lexical91/metrics.json')
    ...
    write(OUT/'receipt.json',{'status':'passed_validation','source_sha256':sha(__file__),'producer_protocol_sha256':sha(BASE/'protocol.json'),
        ...
        'peak_rss_bytes':validation.guard(),'scope':'Tuneonly; exactoriginalcohorts/rawlabels and all4nativepredictions replayed, originalcontrol exact. All22470x15 frozenlexicalfields regenerated fromflighttokens andcomparedexactly withbothcaches. Day/row sensitivity and2000paired-daybootstrap independently computed.'})
