...
OUT = ROOT / 'private_runs/tail240_20260916/validation/alias_oracle_v1'
ID, FID, PHASE, TIME = 'MVT_ID_mvt', 'FLIGHT_ID_mvt', 'ARR_DEP_mvt', 'MVT_TIME_UTC_mvt'
...
def main():
    ...
    for filename, report in summary['months'].items():
        ...
        for left, right in [('mp', 'cp'), ('mn', 'cn'), ('ms', 'cs')]:
            ...
            assert actual.reset_index(drop=True).equals(a.loc[alias, right].reset_index(drop=True))
