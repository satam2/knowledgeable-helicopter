    ...
    for name,(directory,family) in roots.items():
        ...
        for fold in ('F1','F3'):
            ...
            for source in (adapter,encoder):
                ...
                substitute=None
                if not keys:
                    assert name=='lgb225' and source==encoder
                    sibling=base/'deeper_context_union'/f'lightgbm_leaf63_sequence_aobt_allfinite_{fold}_s20260916'/'manifest.json'
                    assert digest in read(sibling)['source_hashes'].values()
                    replay=read(OUT/'lgb225_decoder_receipt.json')
                    assert replay['status']=='passed' and replay['native_max_abs_delta_sec']==0 and replay['rows']==180640
                    substitute=dict(path=str(sibling),manifest_sha256=sha(sibling),native_replay_receipt_sha256=sha(OUT/'lgb225_decoder_receipt.json'),
                                    limitation='Original LGB225 manifests omit imported NativeFrameEncoder source hash. Same frozen encoder bound in sibling union/sequence8 manifests, with exact full-June original225 native replay.')
                checks.append(dict(fold=fold,path=source,sha256=digest,original_manifest_keys=keys,supplemental_evidence=substitute))
