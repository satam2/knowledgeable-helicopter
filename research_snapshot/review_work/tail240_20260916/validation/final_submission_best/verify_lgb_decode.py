...
def main():
    ...
    metadata = pd.read_parquet(ROOT / 'private_runs/screening_230/data/interim/audit/departures.parquet', columns=[ID, 'proxy_sec']).set_index(ID)
    proxy = metadata.loc[expected[ID], 'proxy_sec'].to_numpy(float)
    ...
    for start in range(0,len(expected),8192):
        ...
        prediction[start:stop] = model['estimator'].predict(model['encoder'].transform(frame), num_iteration=model['steps'], num_threads=1) + proxy[start:stop]
