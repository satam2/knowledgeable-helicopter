...
OUT = common.external_path(ROOT / 'private_runs/tail240_20260916/forensics/atfm/tune_v1')
...
def guard(_env=None):
    ...
    assert peak < 6 * 1024**3, f'ATFM 6 GiB process ceiling exceeded: {peak}'
...
def declare():
    ...
    record = dict(source_sha256=common.sha256(__file__),
        ...
        scope='F1 June and F3 October tune only; no score or refit. No ranking labels read.',
        ...
        resources='2 CPU; peak process working set below 6 GiB; host available reserve >=8 GiB. Parent authorizes launch after RAM allocation.',
...
if __name__ == '__main__':
    ...
    if args.declare_only:
        declare()
        print('Declared ATFM416 tune experiment; no fit launched', flush=True)
    else:
        assert args.fold
        run(args.fold)
