...
OUT = common.external_path(ROOT / 'private_runs/tail240_20260916/forensics/atfm/tune_f3_v2')
...
def guard(_env=None):
    ...
    assert memory.rss < 8 * 1024**3 and peak < 8 * 1024**3, f'ATFM F3 8 GiB process ceiling exceeded: RSS={memory.rss}, peak={peak}'
...
def declare():
    ...
    record = dict(source_sha256=common.sha256(__file__),
        ...
        resources='F3 only: 2 CPU, current and OS peak process working set below 8 GiB, host available reserve >=8 GiB. Launch requires >=16 GiB available and explicit parent allocation. F1 measured peak 4.139 GiB after schema cache.',
...
def run(fold):
    ...
    assert psutil.virtual_memory().available >= 16 * 1024**3, 'Launch requires 16 GiB host available'
    guard()
...
if __name__ == '__main__':
    ...
    if args.declare_only:
        declare()
        print('Declared ATFM416 F3 v2 with cached schema discovery and 8 GiB process ceiling; no fit launched', flush=True)
    else:
        assert args.fold
        run(args.fold)
