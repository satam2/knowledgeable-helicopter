...
OUT = common.external_path(ROOT / 'private_runs/tail240_20260916/forensics/atfm/tune_f3_v1')
CONTROL = ROOT / 'private_runs/tail240_20260916/models/following_groups_tune_v2'
...
def sources(desired):
    result, discovery = schema_discovery.cached_discovery(ORIGINAL_SOURCES, desired)
...
def guard(_env=None):
    ...
    assert memory.rss < 16 * 1024**3 and peak < 16 * 1024**3, f'ATFM F3 16 GiB process ceiling exceeded: RSS={memory.rss}, peak={peak}'
...
def declare():
    ...
    record = dict(source_sha256=common.sha256(__file__),
        imported_sources={str(Path(p).relative_to(ROOT)): common.sha256(p) for p in [risk.__file__, addition.__file__, schema_discovery.__file__]},
        ...
        fresh_control_manifest_sha256=common.sha256(CONTROL / 'F3/control387/manifest.json'),
        fresh_control_protocol_sha256=common.sha256(CONTROL / 'protocol.json'),
        global9_preparation_sha256=common.sha256(BASE / 'preparation.json'),
        discovery='Unmodified original source discovery wrapped in once-per-local-path schema cache. Exact base tuple equality against original387 selection and original control feature receipts required; Arrow read_schema restored before model fitting.',
        ...
        control='Reuse freshly fitted following_groups_tune_v2/F3/control387 only after split, IDs, params, feature columns, source integrity and original prediction parity <=1e-7 pass. Missing control stops run.',
        scope='F3 October tune only. F1 final assessment already exposed and primary fixed25 gate failed; F3 is a preplanned second-month comparison, not a fresh selection test. No score or refit. No ranking labels read.',
        ...
        resources='F3 only: 2 CPU, current and OS peak process working set below 16 GiB, host available reserve >=8 GiB. Launch requires >=24 GiB available and explicit parent allocation. F1 measured peak 4.139 GiB after schema cache.',
...
def run(fold):
    protocol = declare()
    assert fold == 'F3', 'This declaration permits F3 only'
    assert psutil.virtual_memory().available >= 24 * 1024**3, 'Launch requires 24 GiB host available'
    guard()
    ...
    assert common.sha256(control_dir / 'manifest.json') == protocol['fresh_control_manifest_sha256']
    assert common.sha256(CONTROL / 'protocol.json') == protocol['fresh_control_protocol_sha256']
    for source_path, expected_hash in control_protocol['sources'].items():
        assert common.sha256(ROOT / source_path) == expected_hash
    ...
    original_tuples, original_discovery = schema_discovery.cached_discovery(ORIGINAL_SOURCES, protocol['columns'][:387])
    candidate_tuples = sources(protocol['columns'])
    assert candidate_tuples[:-1] == original_tuples
    frozen_receipts = [{key: record[key] for key in ['path', 'sha256', 'columns']} for record in marker['feature_receipts']]
    actual_receipts = [dict(path=str(path), sha256=expected, columns=names) for path, expected, fill, names in original_tuples]
    assert actual_receipts == frozen_receipts
    base_dir = ROOT / 'private_runs/screening_230/data/interim/features/a2a101f52a0aa418'
    for path, _, fill, _ in original_tuples:
        expected_fill = path.parent != base_dir and 'sequence_flatten' not in str(path.parent)
        assert fill == expected_fill, 'Original frozen missing-value policy changed'
    assert candidate_tuples[-1][2] is False
    tuple_record = [dict(path=str(path), sha256=expected, fill=fill, columns=names) for path, expected, fill, names in candidate_tuples]
    common.write_json(dest / 'discovery_receipt.json', dict(helper_sha256=common.sha256(schema_discovery.__file__),
        wrapper_sha256=common.sha256(__file__), original_387_selection=original_discovery, candidate_discovery=DISCOVERY_RECEIPTS[-1],
        exact_original_base_tuple_equality=True, exact_control_feature_receipts=True,
        frozen_fill_policy='Base and sequence_flatten: False; other inherited extensions: True; ATFM: False', tuples=tuple_record))
    ...
    common.write_json(dest / 'manifest.json', dict(status='complete', fold=fold, steps=steps,
        ...
        outputs={name: common.sha256(dest / name) for name in ['model.txt', 'tune.parquet', 'encoder.json', 'control_receipt.json', 'discovery_receipt.json']}))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fold', choices=['F3'])
    args = parser.parse_args()
    if args.declare_only:
        declare()
        print('Declared ATFM416 F3 v1 with cached schema discovery and 16 GiB process ceiling; no fit launched', flush=True)
    else:
        assert args.fold
        run(args.fold)
