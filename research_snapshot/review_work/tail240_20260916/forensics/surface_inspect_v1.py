...
OUT = ROOT / 'private_runs/tail240_20260916/forensics/surface_event_pilot/inspection_v1'
...
def main():
    ...
    protocol = dict(source_sha256=common.sha256(__file__), acquisition_manifest_sha256=common.sha256(ACQUIRE / 'manifest.json'),
        ...
        bounds='Exact EVENT.flight_id to January FLIGHT.id. Compare event_time against public first_seen/last_seen, interpreting all source-naive timestamps consistently as UTC.',
    ...
    assert public.id.dtype == np.dtype('uint64') and public.id.is_unique
    ...
    for batch in pq.ParquetFile(event_path).iter_batches(batch_size=8192, use_threads=False):
        ...
        joined = frame.join(public, on='flight_id_exact', how='left', validate='many_to_one')
    ...
    common.write_json(OUT / 'manifest.json', dict(status='complete', source_sha256=common.sha256(__file__), protocol_sha256=common.sha256(OUT / 'protocol.json'),
        rows=total, matched_public_flight_rows=matched, unmatched_or_invalid_bound_rows=total-matched, invalid_event_times=invalid_time,
