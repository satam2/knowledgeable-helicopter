...
OUT = common.external_path(ROOT/'private_runs/tail240_20260916/forensics/source_contract/v1')
pa.set_cpu_count(1)
...
def main():
    ...
    protocol = dict(source_hashes={str(p.relative_to(ROOT)):common.sha256(p) for p in source_paths},
    ...
    with threadpool_limits(1):
        ...
        original = np.memmap(reference_path,mode='r',dtype='float32',shape=matrix.shape,order='F')
        for j in range(len(columns)):
            np.testing.assert_array_equal(matrix[:,j],original[:,j])
            guard()
        del original
        gc.collect()
    ...
    common.write_json(OUT/'receipt.json',dict(status='passed',protocol_sha256=common.sha256(OUT/'protocol.json'),
        ...
        matrix_equal_including_nan=True,feature_values_compared=len(ids)*len(columns),
