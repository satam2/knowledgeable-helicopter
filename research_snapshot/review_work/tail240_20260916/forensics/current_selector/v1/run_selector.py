...
OUT = common.external_path(ROOT / 'private_runs/tail240_20260916/forensics/current_selector/v1')
CAP = 4*1024**3
FIT_SECONDS = 900
...
def declare():
    ...
    sources = [Path(__file__), Path(selector.__file__), Path(__file__).with_name('test_selector.py'),
    ...
    record = dict(status='declared', source_hashes={str(p.relative_to(ROOT)): common.sha256(p) for p in sources},
...
def launch(fold):
    ...
    with (folder/'stdout.log').open('w') as stream:
        ...
        while proc.poll() is None:
            try:
                ...
                maximum = max(maximum,info.rss,getattr(info,'peak_wset',info.rss))
    ...
    common.write_json(folder/'watchdog.json',dict(returncode=returncode,failure=failure,peak_bytes=maximum))
