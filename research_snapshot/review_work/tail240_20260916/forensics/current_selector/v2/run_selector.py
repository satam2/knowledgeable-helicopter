...
OUT = common.external_path(ROOT / 'private_runs/tail240_20260916/forensics/current_selector/v2')
CAP = 4*1024**3
FIT_SECONDS = 900
...
def declare():
    ...
    sources = [Path(__file__), Path(selector.__file__), Path(selector.__file__).with_name('test_selector.py'),
               Path(watchdog.__file__), Path(__file__).with_name('test_watchdog.py'),
    ...
    record = dict(status='declared', preserved_v1_protocol_sha256=common.sha256(OUT.parent/'v1/protocol.json'),
        operational_change='Recursiveprocess-treewatchdog;sumcurrentRSS/conservativesumofperPIDOShistoricalpeaks;killdescendantsbeforelauncher;scienceunchanged', source_hashes={str(p.relative_to(ROOT)): common.sha256(p) for p in sources},
