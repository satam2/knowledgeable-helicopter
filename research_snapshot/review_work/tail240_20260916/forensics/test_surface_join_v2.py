import pandas as pd
import surface_join_v2 as subject


def test_exact_id_ambiguity_and_reuse():
...
def test_microseconds_zero_preservation_and_window():
    assert subject.normalize(pd.Series([' abc001 x ',None])).tolist()==['ABC001X','']
    t=pd.Timestamp('2025-01-06T12:00:00Z')
    public=pd.DataFrame({'match_code':['ABC01'],'adep':['LSZH'],'ades':['EDDF'],
        'first_seen':pd.Series([t-pd.Timedelta(seconds=1800)],dtype='datetime64[us, UTC]'),'flight_id_exact':['123']})
