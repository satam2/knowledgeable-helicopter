...
import pandas as pd
import numpy as np
from run_alias_rotation_models_v2 import extensions,stability,SUFFIXES
...
class ExtensionTests(unittest.TestCase):
    def test_raw_unchanged_and_nonrome_duplicate_blocks(self):
        ...
        raw.index=pd.Index(pd.array(raw.index,dtype="double[pyarrow]"),name=ids.name)
        pilot.index=pd.Index(pd.array(pilot.index,dtype="double[pyarrow]"),name=ids.name)
