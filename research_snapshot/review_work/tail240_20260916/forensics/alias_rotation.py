...
OUT=ROOT / "private_runs/tail240_20260916/forensics/alias_rotation/pilot_v1"
...
def build_month(query,public,mapping,month):
    ...
    codes=dates.query_codes(query,mapping)
    # Raw mode reproduces the old exact-string match, including unparsed codes.
    codes["raw"]=dates.normalize(query.FLIGHT_mvt)
    codes=codes.fillna("")
...
def declare():
    definition={"scope":"Coverage pilot: every Rome missing-NM training DEP, all2025 logicalUTCmonths; no ranking or labels.",
        ...
        "modes":"raw exactnormalized control; observed earlierlogicalUTCmonth mapping; lexicalrepeatedsuffix collapse separately",
