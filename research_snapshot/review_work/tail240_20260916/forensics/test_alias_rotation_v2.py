...
import pandas as pd
import numpy as np
from alias_rotation_v2 import build_month,prefix_codes,ID,TIME
...
class RotationTests(unittest.TestCase):
    def test_prefix_only_preserves_zeros_and_unparsed_codes(self):
        flights=pd.Series(["ITYTY078","ITY078","EXS062C","MABSU",""])
        codes=prefix_codes(flights,{"ITYTY":"ITY"})
        self.assertEqual(codes.learned.tolist(),["ITY078","ITY078","EXS062C","MABSU",""])
        self.assertEqual(codes.lexical.tolist(),["ITY078","ITY078","EXS062C","MABSU",""])
        q=query();q["FLIGHT_mvt"]="ITYTY078"
        p=public();p.loc[1,"flt_id"]="ITY078"
        x,_=build_month(q,p,{"ITYTY":"ITY"},"202507")
        self.assertEqual(x.loc[42,"alias_learned_match"],1)
        q["FLIGHT_mvt"]="MABSU";p.loc[1,"flt_id"]="MABSU"
        x,_=build_month(q,p,{},"202507")
        self.assertEqual(x.loc[42,"alias_raw_match"],1)
        self.assertEqual(x.loc[42,"alias_learned_match"],1)
