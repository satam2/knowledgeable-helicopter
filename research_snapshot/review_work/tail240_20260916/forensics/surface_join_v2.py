...
OUT = ROOT / 'private_runs/tail240_20260916/forensics/surface_event_pilot/join_v2'
...
def unique_matches(query, public):
    ...
    for key, group in public.groupby(['match_code','adep','ades'], sort=False):
        ...
        groups[key] = (ordered.first_seen.dt.as_unit('ns').astype('int64').to_numpy() // 10**9, ordered.flight_id_exact.astype(str).to_numpy())
    records = []
