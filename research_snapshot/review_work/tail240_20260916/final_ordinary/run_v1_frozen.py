...
OUT = common.external_path(ROOT/'private_runs/tail240_20260916/final_ordinary/v1')
REGISTRY = {
...
def declare(name):
    ...
    record = dict(name=name, columns=columns, fold_steps=steps, final_steps=int(np.median(steps)),
        ...
        resource='24GiB current RSS cap,8GiB host reserve,2CPU; one GPU job at a time')
    dest = OUT/name
...
def run(name):
    ...
    assert psutil.virtual_memory().available >= 32*1024**3, 'Full-year fit starts with32GiB available'
