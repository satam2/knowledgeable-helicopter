"""V2 preserves V1 and fixes its absent-label-column reference check before fit."""
import torch
import lightgbm
import argparse
import gc
...
OUT = common.external_path(ROOT / 'private_runs/tail240_20260916/models/catboost_union387_v2')
...
def align_reference(stored, expected_ids):
    assert stored[ID].is_unique and pd.Index(expected_ids).is_unique
    assert len(stored) == len(expected_ids) and set(stored[ID]) == set(expected_ids)
    prediction = stored.set_index(ID).loc[expected_ids, 'prediction_sec'].to_numpy(float)
...
def declare():
    ...
    paths = [Path(__file__), ADAPTER_PATH, Path(sys.modules['encoders'].__file__), Path(context.__file__),
             Path(risk.__file__), Path(schema_discovery.__file__), Path(__file__).with_name('test_catboost_union387_v2.py')]
    bindings = {}
    for fold, marker in controls.items():
    ...
    record = dict(status='declared', preserved_v1_protocol_sha256=common.sha256(OUT.parent / 'catboost_union387_v1/protocol.json'),
        repair='Old prediction parquet has IDs and predictions only; check unique exact IDs and bind labels through original hashed metadata. No scientific model or endpoint change.', source_hashes={str(p.relative_to(ROOT)): common.sha256(p) for p in paths},
...
def fit_fold(fold):
    ...
    try:
        ...
        stored = pd.read_parquet(reference / 'tune_predictions.parquet')
        control_prediction = align_reference(stored, tune[ID])
