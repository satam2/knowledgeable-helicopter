...
OUT = core.external_path(core.ROOT / 'private_runs/breakthrough_20260916/models_retrieval/tabdpt_batch_v3_1')
...
def source_paths():
    return {'v3_run.py': Path(__file__), 'v3_adapter.py': HERE / 'adapter.py',
        'v3_test_contracts.py': HERE / 'test_contracts.py', 'v3_1_tests.py': HERE / 'test_v3_1.py',
...
def capture(model, query, batch):
    ...
    had_instance_override = '_get_context_indices' in model['estimator'].__dict__
    previous_instance_override = model['estimator'].__dict__.get('_get_context_indices')
    contexts = []
    ...
    try:
        ...
        if had_instance_override:
            model['estimator'].__dict__['_get_context_indices'] = previous_instance_override
        else:
        ...
        for batch in [16, 64, 128]:
            ...
            try:
                ...
                if reference_prediction is None:
                    raise AssertionError('Batch16 baseline must pass before larger batches')
            ...
            if batch == 16 and entry['status'] != 'passed':
                raise RuntimeError('Batch16 baseline failed; larger batches cannot become baseline')
