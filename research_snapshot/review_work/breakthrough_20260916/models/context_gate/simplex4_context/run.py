"""Fixed four-context-expert simplex, separate declare/prepare/score stages."""
import os
...
OUT = BASE / 'models/context_gate/simplex4_context_v1'
EXPERTS = ['tabm_source', 'tabm_combined', 'lgb63', 'lgb63_sequence']
...
def declare():
    protocol = {'experts': EXPERTS, 'folds': ['F1', 'F3'], 'threads': 2,
        ...
        'cohort': 'AlloriginalpurgedordinarytunefromJune/October; all4expertmodels originallyfitallfiniteNM.',
        'selection': 'These4expertsfixedbeforeanynewscoreaccess; no coefficientgrids, extrapolation orscoreselectedweights.',
        'stages': 'Declarefirst; prepareonlywhenall4expertscompletebothfolds; savweightsbeforeseparate scorestage.',
...
def prepare(protocol):
    ...
    for fold in ('F1', 'F3'):
        ...
        print('PREPARED_SIMPLEX4', fold, weights, flush=True)
...
def score(protocol):
    ...
    write_json(OUT / 'scoring_protocol.json', {'created_utc': utc_now(), 'source_sha256': sha256(__file__),
        'preparation_sha256': sha256(OUT / 'preparation.json'), 'variants': ['global4', 'airport4'],
    ...
    for fold in ('F1', 'F3'):
        ...
        for variant in ('global4', 'airport4'):
            coefficients = np.tile(weights['global'], (eligible.sum(), 1)) if variant == 'global4' else np.array([weights['airports'].get(a, weights['global']) for a in airports[eligible]])
        ...
        results[fold] = rec
        print('SCORED_SIMPLEX4', fold, {name: row['metrics']['overall']['rmse_sec'] for name, row in variants.items()}, flush=True)
    seasonal = {name: season_score(results['F1']['variants'][name]['metrics']['overall'], results['F3']['variants'][name]['metrics']['overall']) for name in ('global4', 'airport4')}
    ...
    print('SEASONAL_SIMPLEX4', seasonal, flush=True)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--stage', choices=['declare', 'prepare', 'score'], required=True)
