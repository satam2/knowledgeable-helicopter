"""Independent active-set quadratic oracle and saved four-expert predictions."""
import argparse
import itertools
import numpy as np
import pandas as pd
...
def active_set_oracle(y, p, center=None, penalty=0.):
    ...
    for n in range(1, p.shape[1] + 1):
        for active in itertools.combinations(range(p.shape[1]), n):
            ...
            weights /= weights.sum()
            value = (np.square(relative @ weights - target).sum() + penalty * np.square(weights - center).sum()) / scale
...
def synthetic():
    ...
    p = rng.normal(1000, 400, (4096, 4))
    y = p @ np.array([.15, .25, .4, .2]) + rng.normal(0, 10, len(p))
...
def main():
    ...
    for fold in ('F1', 'F3'):
        ...
        for variant in ('global4', 'airport4'):
            ...
            coefficients = np.tile(saved['global'], (len(frame), 1)) if variant == 'global4' else np.array([saved['airports'].get(a, saved['global']) for a in reference.ADEP_mvt.astype(str)])
        ...
        print('INDEPENDENT_SIMPLEX4_VERIFIED', fold, max(gaps.values()), max(coefficient_delta.values()), flush=True)
    for variant in ('global4', 'airport4'):
    ...
    run.write_json(run.OUT / 'independent_verification.json', {'status': 'passed', 'source_sha256': run.sha256(__file__),
        ...
        'oracle': 'All15nonemptyactiveweightsets, independentKKTlinearsolve; coefficients unchanged.', 'gpu_used': False})
