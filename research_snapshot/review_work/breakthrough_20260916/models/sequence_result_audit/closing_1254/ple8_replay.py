"""Independent completed PLE8 CPU replay and full artifact checks."""
import torch
import lightgbm
import sys
...
def main():
    ...
    torch.cuda._lazy_init = lambda *a, **k: (_ for _ in ()).throw(AssertionError('CPU PLE audit must not initialize CUDA'))
    results={}
    base=ROOT/'private_runs/breakthrough_20260916'
    for fold,month,nextmonth in [('F1','07','08'),('F3','11','12')]:
        folder=base/'full_neural'/f'tabm_combined_ple8_aobt_allfinite_{fold}_s20260916'
        ...
        snapshot=base/'full_neural/source_snapshots/tabm_combined_ple8_aobt_allfinite_s20260916'
        for name,digest in record['source_hashes'].items():
            assert fixture.sha256(snapshot/Path(name).name)==digest,name
        assert record['fit']['steps']==min(record['fit']['history'],key=lambda row:row['tune_mse_sec2'])['epoch']
        for variant in ('candidate','blend25'):
            ...
            metric=record['reports'][variant]['metrics']['overall']
            assert metric['n']==len(frame)
            np.testing.assert_allclose(np.square(error).sum(),metric['sse'],rtol=1e-12)
        ...
        results[fold]=result
        print('PLE8_CPU_REPLAY',fold,result['replay_rows'],result['cpu_gpu_max_abs_delta_sec'],flush=True)
        del model,x,extra,saved
        gc.collect()
    (audit.OUT/'closing_1254').mkdir(parents=True,exist_ok=True)
    fixture.write_json(audit.OUT/'closing_1254/ple8_replay.json',{'source_sha256':fixture.sha256(__file__),'folds':results,
