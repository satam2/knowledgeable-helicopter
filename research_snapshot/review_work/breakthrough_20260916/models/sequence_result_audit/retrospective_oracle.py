...
def main():
    ...
    for index, path in enumerate([*paths, RAW / 'ranking.parquet']):
        ...
        gc.collect()
    result.update(status='passed', runtime_sec=time.monotonic() - start, peak_rss_bytes=getattr(psutil.Process().memory_info(), 'peak_wset', psutil.Process().memory_info().rss), independent_mask_and_direct_moments=True, cache_full_output_hashes_verified=True, gpu_used=False)
