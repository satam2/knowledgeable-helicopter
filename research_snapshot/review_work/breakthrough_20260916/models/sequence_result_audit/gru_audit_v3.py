...
def main():
    ...
    for fold, month, nextmonth in [('F1', '07', '08'), ('F3', '11', '12')]:
        ...
        for variant in ('static', 'context'):
            ...
            result = {'status': 'passed' if delta.max() < .05 else 'tolerance_exceeded', 'score_rows': len(saved), 'replay_rows': len(chosen), 'replay_scope': scope,
            ...
            fixture.write_json(audit.OUT / 'gru_audit_v3_progress.json', results)
    ...
    fixture.write_json(audit.OUT / 'gru_audit_v3.json', {'status': 'passed' if all(r['status'] == 'passed' for r in results.values()) else 'cross_device_tolerance_exceeded', 'source_sha256': fixture.sha256(__file__),
